...
def mem_efficient_attention_variable(
    ...
    if in_dygraph_mode():
        return _C_ops.fused_multihead_attention_variable(
            query, key, value, seq_lens, mask, scale, causal
        )

    helper = LayerHelper('fused_multihead_attention_variable', **locals())
    out = helper.create_variable_for_type_inference(dtype=query.dtype)
    helper.append_op(
        type='fused_multihead_attention_variable',
        inputs={'Query': query, 'Key': key, 'Value': value, 'SeqLens': seq_lens, "Mask": mask},
        attrs={"Scale": scale, "Causal": causal},
        outputs={'Out': out},
    )
    return out