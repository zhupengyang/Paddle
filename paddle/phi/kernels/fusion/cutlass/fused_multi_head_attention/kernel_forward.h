...
struct AttentionKernel {
  ...
  struct Params {
    ...
    CUTLASS_DEVICE bool advance_to_block() {
      ...
      output_ptr += int64_t(q_start + query_start) * o_strideM() + head_id * o_strideH;
      ...
      if (output_accum_ptr != nullptr) {
        ...
        output_accum_ptr += int64_t(q_start + query_start) * o_strideM() + head_id * o_strideH;
      } else {
  ...
  static bool __host__ check_supported(Params const& p) {
    ...
    if (p.mask_ptr != nullptr) {
      CHECK_ALIGNED_PTR(p.mask_ptr, kAlignmentM);
      XFORMERS_CHECK(
          p.mask_strideB % kAlignmentM == 0,
          "attn_mask is not correctly aligned");
      XFORMERS_CHECK(
          p.mask_strideH % kAlignmentM == 0,
          "attn_mask is not correctly aligned");
      XFORMERS_CHECK(
          p.mask_strideM % kAlignmentM == 0,
          "attn_mask is not correctly aligned");
    }
    XFORMERS_CHECK(
