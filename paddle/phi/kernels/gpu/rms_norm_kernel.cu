...
template<class Func>
inline cudaError_t GetNumBlocks(Func func, int32_t block_size, size_t dynamic_smem_size,
                                int32_t max_blocks, int32_t waves, int* num_blocks) {
  int dev;
  {
  ...
  *num_blocks =
      std::max<int>(1, std::min<int32_t>(max_blocks, sm_count * max_active_blocks * waves));
  return cudaSuccess;
}
...
__global__ void __launch_bounds__(block_size)RmsNormBlockSMemImpl(LOAD load, STORE store, const int32_t rows,
                                                                    const int32_t cols, const float epsilon, 
...
#pragma unroll
      for (int i = 0; i < pack_size; ++i) {
        pack[i] = static_cast<ComputeType>(buf[i * num_packs + pack_id]) * row_inv_rms;
      }
...
inline cudaError_t LaunchRmsNormBlockSMemImpl(cudaStream_t stream, LOAD load, STORE store,
                                                int smem, const int32_t rows, const int32_t cols,
                                                const float epsilon, 
                                                ComputeType col_divisor) {
  ...
  {
    cudaError_t err =
        GetNumBlocks(RmsNormBlockSMemImpl<LOAD, STORE, ComputeType, pack_size, block_size>,
                     block_size, smem, rows, waves, &grid_dim_x);
    if (err != cudaSuccess) { return err; }
  }
  RmsNormBlockSMemImpl<LOAD, STORE, ComputeType, pack_size, block_size>
      <<<grid_dim_x, block_size, smem, stream>>>(load, store, rows, cols, epsilon, col_divisor);
...
inline cudaError_t TryDispatchRmsNormBlockSMemImplBlockSize(
    cudaStream_t stream, LOAD load, STORE store, const int32_t rows, const int32_t cols,
    const float epsilon , ComputeType col_divisor, bool* success) {
  ...
  static const bool max_smem_configed = [=]() {
    ...
    err = MaximizeDynamicSharedMemorySize(
        RmsNormBlockSMemImpl<LOAD, STORE, ComputeType, pack_size, block_size_conf_1>,
        max_smem_size);
    if (err != cudaSuccess) { return false; }
    err = MaximizeDynamicSharedMemorySize(
        RmsNormBlockSMemImpl<LOAD, STORE, ComputeType, pack_size, block_size_conf_2>,
        max_smem_size);
    if (err != cudaSuccess) { return false; }
    err = MaximizeDynamicSharedMemorySize(
        RmsNormBlockSMemImpl<LOAD, STORE, ComputeType, pack_size, block_size_conf_3>,
        max_smem_size);
    if (err != cudaSuccess) { return false; }
    err = MaximizeDynamicSharedMemorySize(
        RmsNormBlockSMemImpl<LOAD, STORE, ComputeType, pack_size, block_size_conf_4>,
        max_smem_size);
    if (err != cudaSuccess) { return false; }
  ...
  if (max_active_blocks_conf_4 == max_active_blocks_conf_1
    ...
    *success = true;
    return LaunchRmsNormBlockSMemImpl<LOAD, STORE, ComputeType, pack_size, block_size_conf_4>(
  ...
  if (max_active_blocks_conf_3 == max_active_blocks_conf_1
    ...
    *success = true;
    return LaunchRmsNormBlockSMemImpl<LOAD, STORE, ComputeType, pack_size, block_size_conf_3>(
        stream, load, store, smem, rows, cols, epsilon, col_divisor);
  }
  ...
  if (max_active_blocks_conf_2 == max_active_blocks_conf_1
    ...
    *success = true;
    return LaunchRmsNormBlockSMemImpl<LOAD, STORE, ComputeType, pack_size, block_size_conf_2>(
        stream, load, store, smem, rows, cols, epsilon, col_divisor);
  }
...
struct TryDispatchRmsNormBlockSMemImplPackSize {
  cudaError_t operator()(cudaStream_t stream, LOAD load, STORE store, const int32_t rows,
    ...
    if (cols % 4 == 0 && CanPackAs<LOAD>(load, 4) && CanPackAs<STORE>(store, 4)) {
      return TryDispatchRmsNormBlockSMemImplBlockSize<LOAD, STORE, ComputeType, 4>(
          stream, load, store, rows, cols, epsilon, col_divisor, success);
    } else if (cols % 2 == 0 && CanPackAs<LOAD>(load, 2) && CanPackAs<STORE>(store, 2)) {
      return TryDispatchRmsNormBlockSMemImplBlockSize<LOAD, STORE, ComputeType, 2>(
          stream, load, store, rows, cols, epsilon, col_divisor, success);
    } else {
      return TryDispatchRmsNormBlockSMemImplBlockSize<LOAD, STORE, ComputeType, 1>(
          stream, load, store, rows, cols, epsilon, col_divisor, success);
    }
  }
};
...
template<typename LOAD, typename STORE, typename ComputeType>
inline cudaError_t TryDispatchRmsNormBlockSMemImpl(cudaStream_t stream, LOAD load, STORE store,
...
struct SkipLoadAndStoreResidual {
  ...
  SkipLoadAndStoreResidual(SRC* src, 
                           ...
                           float alpha, int32_t row_size)
      : src(src), bias(bias), skip(skip), residual_bias_out(residual_bias_out), alpha(alpha), row_size(row_size) {}
  template<int N>
  __device__ void load(DST* dst, int32_t row, int32_t col) const {
    Pack<SRC, N> src_pack;
...
#pragma unroll
    for (int i = 0; i < N; ++i) {
      ...
      residual_out_pack.elem[i] = src_pack.elem[i]
                                  + bias_pack.elem[i]
                                  + skip_pack.elem[i];
    }
#pragma unroll
...
template<typename SRC, typename DST>
struct AffineStore {
  AffineStore(DST* y, int32_t row_size, const DST* gamma)
      : y(y), row_size(row_size), gamma(gamma){}
  template<int N>
  __device__ void store(const SRC* src, int32_t row, int32_t col) {
    Pack<DST, N> y_pack;
    ...
    for (int i = 0; i < N; ++i) {
      ...
      float normalized_val = normalized_i * static_cast<float>(gamma_pack.elem[i]);
