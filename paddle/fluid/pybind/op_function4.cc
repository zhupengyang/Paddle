...
static PyObject * imperative_cutlass_fused_multihead_attention(PyObject *self, PyObject *args, PyObject *kwargs)
{
  PyThreadState *tstate = nullptr;
  try
  {
    std::string op_type = "cutlass_fused_multihead_attention";
    platform::RecordEvent op_type_record_event("cutlass_fused_multihead_attention pybind_imperative_func");
    
    auto query = GetVarBaseFromArgs(op_type, "query", args, 0, false);
    auto key = GetVarBaseFromArgs(op_type, "key", args, 1, false);
    ...
    imperative::NameVarBaseMap outs = {{"out", {std::shared_ptr<imperative::VarBase>(new imperative::VarBase("auto_"+std::to_string(VarBaseUniqueNameID++)+"_"))}}};
    imperative::NameVarBaseMap ins = {{"query", {query}},{"key", {key}},{"value", {value}}};
...
static PyObject * imperative_fused_multi_transformer(PyObject *self, PyObject *args, PyObject *kwargs)
{
  ...
  try
  {
    ...
    auto PreCaches = GetVarBaseListFromArgs(op_type, "PreCaches", args, 6, true);
    auto RotaryPosEmb = GetVarBaseFromArgs(op_type, "RotaryPosEmb", args, 7, true);
    auto TimeStep = GetVarBaseFromArgs(op_type, "TimeStep", args, 8, true);
    auto SeqLengths = GetVarBaseFromArgs(op_type, "SeqLengths", args, 9, true);
    auto SrcMask = GetVarBaseFromArgs(op_type, "SrcMask", args, 10, true);
    auto OutLinearW = GetVarBaseListFromArgs(op_type, "OutLinearW", args, 11, false);
    auto OutLinearBias = GetVarBaseListFromArgs(op_type, "OutLinearBias", args, 12, true);
    auto FFNLnScale = GetVarBaseListFromArgs(op_type, "FFNLnScale", args, 13, false);
    auto FFNLnBias = GetVarBaseListFromArgs(op_type, "FFNLnBias", args, 14, false);
    auto FFN1Weight = GetVarBaseListFromArgs(op_type, "FFN1Weight", args, 15, false);
    auto FFN1Bias = GetVarBaseListFromArgs(op_type, "FFN1Bias", args, 16, true);
    auto FFN2Weight = GetVarBaseListFromArgs(op_type, "FFN2Weight", args, 17, false);
    auto FFN2Bias = GetVarBaseListFromArgs(op_type, "FFN2Bias", args, 18, true);
    auto QKVWScale = GetVarBaseListFromArgs(op_type, "QKVWScale", args, 19, true);
    auto OutLinearWScale = GetVarBaseListFromArgs(op_type, "OutLinearWScale", args, 20, true);
    auto FFN1WeightScale = GetVarBaseListFromArgs(op_type, "FFN1WeightScale", args, 21, true);
    auto FFN2WeightScale = GetVarBaseListFromArgs(op_type, "FFN2WeightScale", args, 22, true);
    auto CacheKVOut = GetVarBaseListFromArgs(op_type, "CacheKVOut", args, 23, true);
    framework::AttributeMap attrs;
    ConstructAttrMapFromPyArgs(op_type, args, 24, PyTuple_GET_SIZE(args) , attrs);
    tstate = PyEval_SaveThread();
    ...
    if (RotaryPosEmb != nullptr) {
      ins["RotaryPosEmb"] = {RotaryPosEmb};
    }
    ...
    if (SeqLengths != nullptr) {
      ins["SeqLengths"] = {SeqLengths};
    }
    ...
    if (QKVWScale.size() != 0) {
      ins["QKVWScale"] = QKVWScale;
    }

    if (OutLinearWScale.size() != 0) {
      ins["OutLinearWScale"] = OutLinearWScale;
    }

    if (FFN1WeightScale.size() != 0) {
      ins["FFN1WeightScale"] = FFN1WeightScale;
    }

    if (FFN2WeightScale.size() != 0) {
      ins["FFN2WeightScale"] = FFN2WeightScale;
    }
...
static PyObject * imperative_fused_multi_transformer_dyquant(PyObject *self, PyObject *args, PyObject *kwargs)
{
  PyThreadState *tstate = nullptr;
  try
  {
    std::string op_type = "fused_multi_transformer_dyquant";
    platform::RecordEvent op_type_record_event("fused_multi_transformer_dyquant pybind_imperative_func");
    ...
    auto QKVWLLMInt8 = GetVarBaseListFromArgs(op_type, "QKVWLLMInt8", args, 9, false);
    auto OutLinearWLLMInt8 = GetVarBaseListFromArgs(op_type, "OutLinearWLLMInt8", args, 10, false);
    auto FFN1WeightLLMInt8 = GetVarBaseListFromArgs(op_type, "FFN1WeightLLMInt8", args, 11, false);
    auto FFN2WeightLLMInt8 = GetVarBaseListFromArgs(op_type, "FFN2WeightLLMInt8", args, 12, false);
    framework::AttributeMap attrs;
    ConstructAttrMapFromPyArgs(op_type, args, 13, PyTuple_GET_SIZE(args) , attrs);
    tstate = PyEval_SaveThread();
    
    imperative::NameVarBaseMap outs = {{"Out", {std::shared_ptr<imperative::VarBase>(new imperative::VarBase("auto_"+std::to_string(VarBaseUniqueNameID++)+"_"))}}};
    imperative::NameVarBaseMap ins = {{"X", {X}},{"LnScale", LnScale},{"LnBias", LnBias},{"QKVW", QKVW},{"OutLinearW", OutLinearW},{"FFNLnScale", FFNLnScale},{"FFNLnBias", FFNLnBias},{"FFN1Weight", FFN1Weight},{"FFN2Weight", FFN2Weight},{"QKVWLLMInt8", QKVWLLMInt8},{"OutLinearWLLMInt8", OutLinearWLLMInt8},{"FFN1WeightLLMInt8", FFN1WeightLLMInt8},{"FFN2WeightLLMInt8", FFN2WeightLLMInt8}};
...
static PyObject * imperative_moe(PyObject *self, PyObject *args, PyObject *kwargs)
{
  PyThreadState *tstate = nullptr;
  try
  {
    std::string op_type = "moe";
    platform::RecordEvent op_type_record_event("moe pybind_imperative_func");
    
    auto X = GetVarBaseFromArgs(op_type, "X", args, 0, false);
    auto Gate = GetVarBaseFromArgs(op_type, "Gate", args, 1, false);
    auto Bmm0 = GetVarBaseFromArgs(op_type, "Bmm0", args, 2, false);
    auto Bias0 = GetVarBaseFromArgs(op_type, "Bias0", args, 3, false);
    auto Bmm1 = GetVarBaseFromArgs(op_type, "Bmm1", args, 4, false);
    auto Bias1 = GetVarBaseFromArgs(op_type, "Bias1", args, 5, false);
    framework::AttributeMap attrs;
    ConstructAttrMapFromPyArgs(op_type, args, 6, PyTuple_GET_SIZE(args) , attrs);
    tstate = PyEval_SaveThread();
    
    imperative::NameVarBaseMap outs = {{"Out", {std::shared_ptr<imperative::VarBase>(new imperative::VarBase("auto_"+std::to_string(VarBaseUniqueNameID++)+"_"))}}};
    imperative::NameVarBaseMap ins = {{"X", {X}},{"Gate", {Gate}},{"Bmm0", {Bmm0}},{"Bias0", {Bias0}},{"Bmm1", {Bmm1}},{"Bias1", {Bias1}}};
...
static PyMethodDef ExtestMethods[] = {
  ...
  {"cutlass_fused_multihead_attention", (PyCFunction)(void(*)(void))imperative_cutlass_fused_multihead_attention, METH_VARARGS | METH_KEYWORDS, "C++ interface function for cutlass_fused_multihead_attention in dygraph."},
  ...
  {"fused_multi_transformer_dyquant", (PyCFunction)(void(*)(void))imperative_fused_multi_transformer_dyquant, METH_VARARGS | METH_KEYWORDS, "C++ interface function for fused_multi_transformer_dyquant in dygraph."},
  ...
  {"moe", (PyCFunction)(void(*)(void))imperative_moe, METH_VARARGS | METH_KEYWORDS, "C++ interface function for moe in dygraph."},
