...
static PyObject * imperative_npu_identity(PyObject *self, PyObject *args, PyObject *kwargs)
{
  PyThreadState *tstate = nullptr;
  try
  {
    std::string op_type = "npu_identity";
    platform::RecordEvent op_type_record_event("npu_identity pybind_imperative_func");
...
static PyObject * imperative_warprnnt(PyObject *self, PyObject *args, PyObject *kwargs)
{
  PyThreadState *tstate = nullptr;
  try
  {
    std::string op_type = "warprnnt";
    platform::RecordEvent op_type_record_event("warprnnt pybind_imperative_func");
    
    auto input = GetVarBaseFromArgs(op_type, "input", args, 0, false);
    auto label = GetVarBaseFromArgs(op_type, "label", args, 1, false);
    auto input_lengths = GetVarBaseFromArgs(op_type, "input_lengths", args, 2, false);
    auto label_lengths = GetVarBaseFromArgs(op_type, "label_lengths", args, 3, false);
    framework::AttributeMap attrs;
    ConstructAttrMapFromPyArgs(op_type, args, 4, PyTuple_GET_SIZE(args) , attrs);
    tstate = PyEval_SaveThread();
    
    imperative::NameVarBaseMap outs = {{"loss", {std::shared_ptr<imperative::VarBase>(new imperative::VarBase("auto_"+std::to_string(VarBaseUniqueNameID++)+"_"))}},{"warprnntgrad", {std::shared_ptr<imperative::VarBase>(new imperative::VarBase("auto_"+std::to_string(VarBaseUniqueNameID++)+"_"))}}};
    imperative::NameVarBaseMap ins = {{"input", {input}},{"label", {label}},{"input_lengths", {input_lengths}},{"label_lengths", {label_lengths}}};
    
    imperative::GetCurrentTracer()->TraceOp(op_type, ins, outs, attrs, {});
    PyEval_RestoreThread(tstate);
    tstate = nullptr;
    return MakeReturnPyObject(std::make_tuple(outs["loss"][0],outs["warprnntgrad"][0]));
  }
  catch(...) {
    if (tstate) {
      PyEval_RestoreThread(tstate);
...
static PyMethodDef ExtestMethods[] = {
  ...
  {"npu_identity", (PyCFunction)(void(*)(void))imperative_npu_identity, METH_VARARGS | METH_KEYWORDS, "C++ interface function for npu_identity in dygraph."},
  ...
  {"warprnnt", (PyCFunction)(void(*)(void))imperative_warprnnt, METH_VARARGS | METH_KEYWORDS, "C++ interface function for warprnnt in dygraph."},
