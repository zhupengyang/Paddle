...
static PyObject * imperative_top_p_sampling(PyObject *self, PyObject *args, PyObject *kwargs)
{
  PyThreadState *tstate = nullptr;
  try
  {
    std::string op_type = "top_p_sampling";
    platform::RecordEvent op_type_record_event("top_p_sampling pybind_imperative_func");
    
    auto x = GetVarBaseFromArgs(op_type, "x", args, 0, false);
    auto ps = GetVarBaseFromArgs(op_type, "ps", args, 1, false);
    framework::AttributeMap attrs;
    ConstructAttrMapFromPyArgs(op_type, args, 2, PyTuple_GET_SIZE(args) , attrs);
    tstate = PyEval_SaveThread();
    
    imperative::NameVarBaseMap outs = {{"out", {std::shared_ptr<imperative::VarBase>(new imperative::VarBase("auto_"+std::to_string(VarBaseUniqueNameID++)+"_"))}},{"ids", {std::shared_ptr<imperative::VarBase>(new imperative::VarBase("auto_"+std::to_string(VarBaseUniqueNameID++)+"_"))}}};
    imperative::NameVarBaseMap ins = {{"x", {x}},{"ps", {ps}}};
    
    imperative::GetCurrentTracer()->TraceOp(op_type, ins, outs, attrs, {});
    PyEval_RestoreThread(tstate);
    tstate = nullptr;
    return MakeReturnPyObject(std::make_tuple(outs["out"][0],outs["ids"][0]));
  }
  catch(...) {
    if (tstate) {
      PyEval_RestoreThread(tstate);
...
static PyMethodDef ExtestMethods[] = {
  ...
  {"top_p_sampling", (PyCFunction)(void(*)(void))imperative_top_p_sampling, METH_VARARGS | METH_KEYWORDS, "C++ interface function for top_p_sampling in dygraph."},
