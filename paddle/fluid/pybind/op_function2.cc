...
static PyObject * imperative_fused_conv2d(PyObject *self, PyObject *args, PyObject *kwargs)
{
  PyThreadState *tstate = nullptr;
  try
  {
    std::string op_type = "fused_conv2d";
    platform::RecordEvent op_type_record_event("fused_conv2d pybind_imperative_func");
...
static PyObject * imperative_fused_gemm_epilogue_grad(PyObject *self, PyObject *args, PyObject *kwargs)
{
  PyThreadState *tstate = nullptr;
  try
  {
    std::string op_type = "fused_gemm_epilogue_grad";
    platform::RecordEvent op_type_record_event("fused_gemm_epilogue_grad pybind_imperative_func");
    
    auto DOut = GetVarBaseFromArgs(op_type, "DOut", args, 0, false);
    ...
    imperative::NameVarBaseMap outs = {{"DY", {std::shared_ptr<imperative::VarBase>(new imperative::VarBase("auto_"+std::to_string(VarBaseUniqueNameID++)+"_"))}}};
    imperative::NameVarBaseMap ins = {{"DOut", {DOut}},{"X", {X}},{"Y", {Y}}};
    
    imperative::GetCurrentTracer()->TraceOp(op_type, ins, outs, attrs, {});
    PyEval_RestoreThread(tstate);
    tstate = nullptr;
    return MakeReturnPyObject(outs["DY"][0]);
  }
  catch(...) {
    if (tstate) {
      PyEval_RestoreThread(tstate);
...
static PyMethodDef ExtestMethods[] = {
  ...
  {"fused_conv2d", (PyCFunction)(void(*)(void))imperative_fused_conv2d, METH_VARARGS | METH_KEYWORDS, "C++ interface function for fused_conv2d in dygraph."},
  ...
  {"fused_gemm_epilogue_grad", (PyCFunction)(void(*)(void))imperative_fused_gemm_epilogue_grad, METH_VARARGS | METH_KEYWORDS, "C++ interface function for fused_gemm_epilogue_grad in dygraph."},
