...
inline __device__ float2 rotary_embedding_transform(const float2 v, const float2 cos, const float2 sin)
{
  float2 rot_v;
...
inline __device__ uint32_t rotary_embedding_transform(const uint32_t v, const uint32_t cos, const uint32_t sin)
{
  float2 fv = half2_to_float2(v);
...
inline __device__ __nv_bfloat162 rotary_embedding_transform(const __nv_bfloat162 v, const __nv_bfloat162 cos, const __nv_bfloat162 sin)
{
  float2 fv = bf1622float2(v);
...
inline __device__ void apply_rotary_embedding(float& q, float& k, float& cos, float& sin)
{
  return;
}

inline __device__ void apply_rotary_embedding(float2& q, float2& k, float2& cos, float2& sin)
{
  q = rotary_embedding_transform(q, cos, sin);
  k = rotary_embedding_transform(k, cos, sin);
}

inline __device__ void apply_rotary_embedding(float4& q, float4& k, float4& cos, float4& sin)
{
...
inline __device__ void apply_rotary_embedding(uint32_t& q, uint32_t& k, uint32_t& cos, uint32_t& sin)
{
  q = rotary_embedding_transform(q, cos, sin);
  k = rotary_embedding_transform(k, cos, sin);
}
...
#ifdef ENABLE_BF16
inline __device__ void apply_rotary_embedding(__nv_bfloat162& q, __nv_bfloat162& k, __nv_bfloat162& cos, __nv_bfloat162& sin)
{
  q = rotary_embedding_transform(q, cos, sin);
  k = rotary_embedding_transform(k, cos, sin);
}
